...
async def _server_propose_turn(sender_id: str, text: str, channel: str, conversation_id,
    ...
    try:
        ...
        if not provider_message_id:
            ...
            return None
        # CA 235-02 + 236-01: CLAIM su kien provider (INBOX state machine) TRUOC extraction. 'succeeded'/'busy'
        # -> no-op (replay on dinh / concurrent). 'claimed' -> xu ly + PHAI mark ket qua (succeeded/retryable)
        # de EFFECTIVE-ONCE: loi transient truoc mutation ben vung KHONG lam mat event vinh vien.
        from app.db_pool import acquire as _acq
        from app.db_pool import release as _rel
        from app.services.command import order_intent_service as _svc

        async def _mark(ok: bool) -> None:
            _mc = await _acq()
            try:
                await _svc.mark_inbound_event(_mc, channel=channel,
                                              provider_message_id=provider_message_id, ok=ok)
            finally:
                await _rel(_mc)
        ...
        try:
            async with _c.transaction():
                _claim = await _svc.claim_inbound_event(
                    _c, channel=channel, provider_message_id=provider_message_id)
        finally:
            await _rel(_c)
        if _claim in ("succeeded", "busy"):
            return {"replay_noop": True}  # da xu ly xong / concurrent -> ZERO mutation
        # _claim == 'claimed' -> tu day moi loi/return PHAI mark ket qua.
        try:
            ...
            # Khong trich duoc field + khong clear -> stable no-op (chat thuong): mark succeeded, KHONG retry.
            ...
            result = await _execute_tool("create_order", args, sender_id, text, command_ctx=cmd_ctx)
            # error/fail-closed -> retryable (redelivery duoc thu lai); moi outcome khac -> succeeded.
            _ok = not (isinstance(result, dict) and (result.get("error") or result.get("intent_fail_closed")))
            await _mark(_ok)
            return result
        except Exception:
            await _mark(False)  # loi truoc mutation ben vung -> retryable, KHONG mat event
            raise
