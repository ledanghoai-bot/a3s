...
async def try_server_commit(*, customer_id: int | None, conversation_id, channel: str, actor_id: str,
    ...
    if row.get("summary_version") != row["state_version"] or \
        ...
        return None
    # CA 234-02.4: REVALIDATE draft (dang + SKU TON TAI) NGAY TRUOC commit — khong chi draft_complete. San
    # pham co the da bi xoa/doi sau khi present summary. Sai -> KHONG commit (fail-closed).
    _conn2 = None
    try:
        _conn2 = await acquire()
        if await _validate_ready(_conn2, row):
            return None
        # CA 234-03: persisted server-response PHAI khop content_hash cua summary render lai (deterministic)
        # -> pending-confirm tuong ung dung response da luu. Lech -> khong commit.
        if row.get("summary_content_hash") is not None:
            _rendered = await _render_confirmation_summary(_conn2, row)
            if _summary_hash(_rendered) != row.get("summary_content_hash"):
                return None
    finally:
