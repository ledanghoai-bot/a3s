...
async def claim_inbound_event(conn, *, channel: str, provider_message_id: str) -> bool:
    """CA 235-02: CLAIM su kien provider dau vao (channel, provider_message_id) — durable, scoped theo kenh
    + danh tinh provider. Tra True neu MOI (chua xu ly, duoc phep chay extraction/propose); False neu REPLAY
    (da claim truoc do) -> caller no-op, KHONG extraction lai/mutation. ON CONFLICT DO NOTHING (idempotent)."""
    r = await conn.fetchval(
        "INSERT INTO inbound_event_log(channel, provider_message_id) VALUES($1,$2) "
        "ON CONFLICT (channel, provider_message_id) DO NOTHING RETURNING channel",
