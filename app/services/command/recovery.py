...
async def retry_outbox(outbox_id: str, actor: dict, reason: str | None) -> dict:
    ...
    try:
        async with conn.transaction():
            ...
            await conn.execute(
                "UPDATE outbox_events SET status='retry_scheduled', available_at=now(), attempt_count=0, "
...
async def replay_outbox(outbox_id: str, actor: dict, reason: str | None) -> dict:
    """Tao outbox event MOI tu event cu (admin, §9.3). Dedupe key moi de gui lai. Audit fail-closed."""
    reason = _require_reason(reason)
    conn = await acquire()
    try:
        async with conn.transaction():
            src = await conn.fetchrow(
                "SELECT command_id, event_type, event_version, destination, dedupe_key, payload, "
                "max_attempts FROM outbox_events WHERE id=$1", outbox_id)
            if src is None:
                raise errors.CommandError("outbox_not_found", "Khong tim thay.", http_status=404)
            ...
            atype, aref, asid = _audit_actor(actor)
            await audit_service.record(conn, atype, "outbox.replay", actor_ref=aref, actor_staff_id=asid,
                                       entity_type="outbox_event", entity_id=str(outbox_id), reason=reason,
