...
async def _validate_for_bind(conn, order_id, resolution_id, actor, reason, ticket):
    """Chung cho bind_order + bind_in_order_tx: require/audit-provision/idempotency/verified/owner/provenance.
    Tra ('idempotent', snapshot_dict, None) neu order da bind DUNG resolution do; ('apply', res, prov) neu san
    sang ghi. Raise BindingError (fail-closed) moi truong hop khong hop le. KHONG mo transaction o day."""
    ...
    if ex:
        ...
        if str(ex["resolution_id"]) == str(resolution_id):
            return ("idempotent", _snap(ex), None)  # idempotent (1 snapshot/order)
    ...
    return ("apply", res, prov)


async def _write_snapshot(conn, *, order_id, res, prov, resolution_id, actor, reason, ticket) -> dict:
    """Ghi snapshot bat bien + UPDATE orders + audit. KHONG mo transaction — chay tren conn cua caller
    (bind_order boc trong tx rieng; Gate E chay TRONG tx tao don). Bat ky loi nao -> propagate -> caller
    rollback."""
...
async def bind_order(conn, *, order_id: int, resolution_id: str, actor: str, reason: str, ticket: str,
    ...
    orders.customer_id (khong tin body — CA Review 117). Mo transaction RIENG khi apply (goi doc lap)."""
    kind, a, prov = await _validate_for_bind(conn, order_id, resolution_id, actor, reason, ticket)
    if kind == "idempotent":
        return a
    res = a
    if not apply:
    ...
    async with conn.transaction():
        return await _write_snapshot(conn, order_id=order_id, res=res, prov=prov,
                                     resolution_id=resolution_id, actor=actor, reason=reason, ticket=ticket)


async def bind_in_order_tx(conn, *, order_id: int, resolution_id: str, actor: str, reason: str,
                           ticket: str) -> dict:
    """M5 Gate E (CA Directive 190): bind TRONG transaction tao don dang mo cua caller — KHONG mo tx long
    nhau. Snapshot INSERT + UPDATE orders + audit nam cung tx tao don; bat ky loi nao (validate/insert/
    immutable/audit) propagate ra _run_winner -> rollback CA don (khong order mo coi, khong snapshot le).
    Ownership derive server-side (orders.customer_id), khong tin body — CA Review 117 + Directive 190 §4.1."""
    kind, a, prov = await _validate_for_bind(conn, order_id, resolution_id, actor, reason, ticket)
    if kind == "idempotent":
        return a
    res = a
