...
async def deliver_once(conn, *, outbox_id: str, transport, backoff_seconds: int = 60) -> dict:
    """Thu gui MOT lan qua transport LOCAL. transport(payload) tra binh thuong = thanh cong; raise = that bai.
    Success -> state='sent'. Fail -> attempts++ , dead_letter neu >= max_attempts, nguoc lai 'failed' + backoff.
    Terminal (sent/dead_letter) -> idempotent no-op. Provider fail KHONG bao gio mark 'sent'."""
    o = await conn.fetchrow("SELECT * FROM address_confirmation_outbox WHERE id=$1::uuid", str(outbox_id))
    if not o:
        raise OutboxError("outbox item khong ton tai")
    o = dict(o)
    if o["state"] in ("sent", "dead_letter"):
        return _row(o)  # terminal — idempotent
    payload = o["payload"]
    ...
    return _row(await conn.fetchrow("SELECT * FROM address_confirmation_outbox WHERE id=$1::uuid", str(outbox_id)))
