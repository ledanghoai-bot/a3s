"""CA Directive 305 §6 — Shop Settings integrations API. RBAC granular per-action; readback REDACTED (khong
ciphertext/plaintext). Secret write-only. Mutation qua service (audit + version CAS + fail-closed)."""
...
def _map_err(e: Exception) -> HTTPException:
    msg = str(e)
    code = 409 if ("version conflict" in msg or "da doi" in msg) else 400
...
async def write_secret(integration_id: int, body: dict, staff: dict = Depends(require_active_session)) -> dict:
    ...
    # value rong/omitted = GIU NGUYEN (CA 304-07 blank-keeps) -> no-op, khong loi.
    ...
    try:
        async with conn.transaction():
            return await svc.write_secret(conn, integration_id, key_name=key_name, plaintext=value, actor=_actor(staff))
    except svc.SettingsError as e:
        raise _map_err(e)
...
@router.delete("/integrations/{integration_id}/secret/{key_name}",
               dependencies=[Depends(require_permission(_P_SECRET))])
async def clear_secret(integration_id: int, key_name: str, staff: dict = Depends(require_active_session)) -> dict:
    conn = await asyncpg.connect(_db_url())
    try:
        async with conn.transaction():
            await svc.clear_secret(conn, integration_id, key_name=key_name, actor=_actor(staff))
        return {"key_name": key_name, "cleared": True}
    finally:
        await conn.close()
...
async def enable(integration_id: int, body: dict, staff: dict = Depends(require_active_session)) -> dict:
    ...
    try:
        async with conn.transaction():
            return await svc.enable(conn, integration_id, expected_version=ev, actor=_actor(staff))
    except svc.SettingsError as e:
        raise _map_err(e)
    finally:
        await conn.close()
...
async def disable(integration_id: int, staff: dict = Depends(require_active_session)) -> dict:
    conn = await asyncpg.connect(_db_url())
    try:
        async with conn.transaction():
            return await svc.disable(conn, integration_id, actor=_actor(staff))
    except svc.SettingsError as e:
        raise _map_err(e)
    finally:
        await conn.close()
...
async def archive(integration_id: int, staff: dict = Depends(require_active_session)) -> dict:
    conn = await asyncpg.connect(_db_url())
    try:
        async with conn.transaction():
            await svc.archive(conn, integration_id, actor=_actor(staff))
