...
async def mk_ready_draft(cid,conv,sku='SPCC',qty=1,name='H',phone='0900007000',addr='12 Le Loi, Ea Kao'):
    ...
    try:
        async with c.transaction():
            ...
            # CA 235-01: arm dung contract — persist summary response row (dedupe) + content_hash mandatory.
            import hashlib
            summary=f"Xac nhan don {sku} x{qty} - {name}"
            await svc.log_message_tx(c,conv,'bot',summary,dedupe_key=f"order_summary:{it['id']}:{r['state_version']}")
