...
async def main():
    ...
    # ---------- 233-04/235-02: TRUE provider-event replay after COMMITTED -> no new intent/order ----------
    # Redeliver the SAME order event id (claim dedupe) -> zero mutation. (Order message = 1 event id; confirm
    # dung event khac.) Full order voi event MOI = don moi hop le (235-02b) — nen replay phai CUNG event id.
    cidR = await setup(f"tg:v2r-{RUN}")
    await enroll(cidR)
    PMID_ORD = f"tg:{RUN}9001"  # numeric suffix — verify_and_link yeu cau tg:<so>
    _EX.clear()
    _EX.append(dict(FULL))
    _S.clear()
    _S.append({"text": "ok"})
    await orchestrator.handle_message(f"tg:v2r-{RUN}", "đặt 1 hũ cà phê giao Ea Kao Hoa 0900001234",
                                      channel="telegram_customer", provider_message_id=PMID_ORD)
    ...
    _EX.clear()
    _EX.append(dict(FULL))  # would-be new order, but SAME event id -> replay claim -> no-op
    _S.clear()
    _S.append({"text": "Dạ ạ."})
    await orchestrator.handle_message(f"tg:v2r-{RUN}", "đặt 1 hũ cà phê giao Ea Kao Hoa 0900001234",
                                      channel="telegram_customer", provider_message_id=PMID_ORD)
    ck("233-04/235-02 TRUE replay (cung event id) sau COMMITTED -> khong intent/order moi",
