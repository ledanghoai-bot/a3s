...
async def main():
    ...
    _EX.clear()
    _EX.append({"quantity": 2})  # PARTIAL (no sku), khong explicit reorder
    _S.clear()
    _S.append({"text": "..."})
    await send(f"tg:p5-{RUN}", "đặt hàng số lượng 2")  # 'dat hang' = order marker -> ENTERS proposal path
    ck("234-02.5 partial stale sau COMMITTED (ENTERS path, khong reorder) -> KHONG intent/order moi",
    ...
    # Scope theo customer cua RUN nay (shared m5lab DB — tranh orphan co y tu 235-01c cua run khac).
    orphan = await q1("SELECT count(*) FROM order_intents oi JOIN customers c ON c.id=oi.customer_id "
                      "WHERE c.psid LIKE $1 AND oi.state='READY_TO_COMMIT' "
                      ...
                      "'order_summary:'||oi.id||':'||oi.summary_version)", f"%{RUN}")
