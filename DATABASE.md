...
> Cập nhật lần cuối: 16/7 (sau migration 007). Nếu thêm migration mới, nhớ cập
> nhật file này theo.
...
knowledge_chunks — độc lập, dùng cho RAG (kiến thức sản phẩm tĩnh), không
liên kết tới khách hàng nào.
```
...
| `products` | Danh mục sản phẩm (hiện chỉ 1 SKU: `3S-100G`) |
...
| `knowledge_chunks` | Kiến thức sản phẩm cho RAG (vector embedding, pgvector) |
...
|---|---|---|
| `id` | BIGSERIAL PK | |
| `sku` | TEXT UNIQUE | Hiện chỉ có `3S-100G` |
...
|---|---|---|
| `id` | BIGSERIAL PK | |
| `source` | TEXT | Tên file nguồn (`product_profile.md`, `faq.md`...) |
| `content` | TEXT | Đoạn text đã chunk |
| `embedding` | `vector(384)` | Model: `paraphrase-multilingual-MiniLM-L12-v2`. Index HNSW cosine. |
| `created_at` | TIMESTAMPTZ | |

Ghi lại qua `scripts/ingest.py`. **Không liên kết với khách hàng nào** — đây
là kiến thức sản phẩm tĩnh dùng chung cho mọi cuộc chat (RAG).
...
```
Khi deploy môi trường **hoàn toàn mới** (volume Postgres trống), cả 7 file sẽ
